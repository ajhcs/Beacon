...
use fresnel_fir_ir::expr::{Expr, FnClassification, Literal, OpKind, QuantifierKind};
...
impl TypeContext {
    pub fn from_ir(ir: &FresnelFirIR) -> Self {
        let mut entities = HashMap::new();
        for (name, entity) in &ir.entities {
            let mut fields = HashMap::new();
            for (fname, fdef) in &entity.fields {
                let field_type = match &fdef.field_type {
                    fresnel_fir_ir::types::FieldType::String { .. } => "string",
                    fresnel_fir_ir::types::FieldType::Bool { .. } => "bool",
                    fresnel_fir_ir::types::FieldType::Int { .. } => "int",
                    fresnel_fir_ir::types::FieldType::Enum { .. } => "enum",
                    fresnel_fir_ir::types::FieldType::Ref { .. } => "ref",
                };
                fields.insert(
                    fname.clone(),
                    FieldInfo {
        ...
        for (name, func) in &ir.functions {
            let classification = match func.classification {
                fresnel_fir_ir::types::FnClassification::Derived => "derived",
                fresnel_fir_ir::types::FnClassification::Observer => "observer",
            };
            functions.insert(
                name.clone(),
                FunctionInfo {
