...
use beacon_compiler::predicate::CompiledExpr;
use beacon_ir::expr::{OpKind, QuantifierKind};
...
/// Convert from beacon-compiler's Value to beacon-model's Value.
fn compiler_value_to_model_value(v: &beacon_compiler::predicate::Value) -> Value {
    match v {
        beacon_compiler::predicate::Value::Bool(b) => Value::Bool(*b),
        beacon_compiler::predicate::Value::Int(i) => Value::Int(*i),
        beacon_compiler::predicate::Value::String(s) => Value::String(s.clone()),
    }
}
