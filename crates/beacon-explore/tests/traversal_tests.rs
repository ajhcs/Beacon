use beacon_compiler::graph::{BranchEdge, GraphNode, NdaGraph};
use beacon_explore::traversal::engine::{ModelOnlyExecutor, TraversalEngine};
use beacon_explore::traversal::runner::{run_campaign, CampaignConfig};
use beacon_explore::traversal::signal::SignalType;
use beacon_explore::traversal::strategy::{PseudoRandomStrategy, StrategyStack};
use beacon_explore::traversal::trace::TraceStepKind;
use beacon_explore::traversal::vector_source::MockVectorSource;
use beacon_explore::traversal::weight_table::WeightTable;
use beacon_ir::types::BeaconIR;
use beacon_model::state::{InstanceId, ModelState, Value};
use rand::SeedableRng;
use rand_chacha::ChaCha8Rng;

fn minimal_ir() -> BeaconIR {
    serde_json::from_str(
        r#"{
            "entities": {},
            "refinements": {},
...
fn test_effects_applied_during_traversal() {
    ...
    let ir: BeaconIR = serde_json::from_str(
        r#"{
            "entities": {
                "Document": {
                    "fields": {
